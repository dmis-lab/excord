...
from excord import consistency_reg
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            orig_loss = outputs[0]
            inputs_rewrite = copy.deepcopy(inputs)
            inputs_rewrite['input_ids'] = batch[8]
            query_end = batch[9]
            
            rewr_loss, cons_loss = consistency_reg(args, 
                                            inputs_rewrite, outputs[1:], model,
                                            query_end
                                        )

            loss_value = {'orig_loss' : orig_loss.item(), 
                            'rewr_loss' : rewr_loss.item(),
                            'cons_loss' : cons_loss.item()}
            
            loss = args.excord_orig_coeff * orig_loss + \
                    args.excord_rewr_coeff * rewr_loss + \
                    args.excord_cons_coeff * cons_loss
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if evaluate:
            ...
            examples = processor.get_train_examples(args.data_dir, filename=args.train_file, excord=True)
...
def main():
    ...
    parser.add_argument('--excord_orig_coeff',type=float, default=1.0, help="Coefficient value of the original loss")
    parser.add_argument('--excord_rewr_coeff',type=float, default=0.5, help="Coefficient value of the loss when models are given the rewritten questions")
    parser.add_argument('--excord_cons_coeff',type=float, default=0.5, help="Coefficient value of the consistency loss")
    parser.add_argument('--excord_conf_thres_span',type=float, default=-1, 
                        help="Confidence-based masking for span prediction")
    parser.add_argument('--excord_conf_thres_cls',type=float, default=-1, 
                        help="Confidence-based masking for classification")
    parser.add_argument('--excord_softmax_temp',type=float, default=-1, 
                        help="Temperature value for the logits of models given the rewritten questions")
